...
class AccountingReport(models.TransientModel):
    ...
    account_report_id = fields.Many2one('account.financial.report', string='Reportes de cuenta', required=True, default=_get_account_report)
    ...
    filter_cmp = fields.Selection([('filter_no', 'Sin filtros'), ('filter_date', 'Fecha')], string='Filtrado por', required=True, default='filter_no')
    date_from_cmp = fields.Date(string='Fecha de inicio')
    date_to_cmp = fields.Date(string='Fecha de fin')
    debit_credit = fields.Boolean(string='Visualizar columnas de débito / crédito', help="This option allows you to get more details about the way your balances are computed. Because it is space consuming, we do not allow to use it while doing a comparison.")
